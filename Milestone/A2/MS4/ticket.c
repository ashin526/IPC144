...
//Function 5: load accounts from tickets.txt
int loadTickets(struct Ticket ticket[], int size){
    int i = 0;
    int j = 0;
    FILE *fp = NULL;
    fp = fopen("tickets.txt", "r"); // read to textfile
    if (fp != NULL){
...
//Function 6 : update all tickets to tickets.txt file
int saveTickets(struct Ticket *tickets, int sizeTicket){
    int i = 0;
    int j = 0;
    int count = 0;
    FILE *fp = NULL;
    ...
    if (fp != NULL){
        for (i = 0; i < sizeTicket; i++){
            if(tickets[i].unique_no !=0){
            fprintf(fp, "%d|%d|%d|%s|%d|", 
            tickets[i].unique_no, tickets[i].account_no, tickets[i].ticket_indicator, tickets[i].subject, tickets[i].counter);
                for (j = 0; j < tickets[i].counter; j++){
                    fprintf(fp, "%c|%s|%s|", tickets[i].msg[j].account_type, tickets[i].msg[j].display_name, tickets[i].msg[j].message);
                }
            fprintf(fp, "\n");
            count ++;
            }
