...
//Function 7: Display's that password is applied through password validation.
void getPaswordCriteria(struct UserLogin* accountLogin){
    int digit, upper, lower, symbol, i;
    char ch;
    do{
        getCString(accountLogin->password, PASSWORD, PASSWORD);
        digit = 0;
        upper = 0;
        lower = 0;
        symbol = 0;
...
//Function 7: load accounts from accounts.txt
int loadAccounts(struct Account accounts[], int size){
    int i=0;
    FILE *fp;
    fp = fopen("accounts.txt", "r"); // read to textfile
    if(!fp) {
        printf("Can't open the file!\n");
        return -1;
    }

    while (fscanf(fp, "%d~%c", &accounts[i].account_no, &accounts[i].account_type) != EOF && i < size){
        if(accounts[i].account_type == 'C'){   
            fscanf(fp, "~%[^~]~%d~%lf~%[^\n]",
            accounts[i].Person.fullname, &accounts[i].Person.birth, &accounts[i].Person.income, accounts[i].Person.country);
        }
        else
            fscanf(fp, "~%[^~]~%d~%lf~%[^~]~%[^~]~%[^\n]", accounts[i].Person.fullname, &accounts[i].Person.birth, 
            &accounts[i].Person.income, accounts[i].Person.country, accounts[i].Login.login, accounts[i].Login.password);
            i++;
    }
    fclose(fp);
    return i;
...
//Function 8 : update all accounts to accounts.txt file
int saveAccounts(struct Account accounts[], int sizeAccount){
    ...
    if (fp != NULL){
        for (i = 0; i < sizeAccount; i++){
            if (accounts[i].account_no != 0){

                fprintf(fp, "%d~%c~%s~%d~%.2lf~%s~%s~%s\n",
                        accounts[i].account_no, accounts[i].account_type, accounts[i].Person.fullname, accounts[i].Person.birth, accounts[i].Person.income,
                        accounts[i].Person.country, accounts[i].Login.login, accounts[i].Login.password);
                if (accounts[i].account_no != 0){
                    accountsave++;
                }
            }
